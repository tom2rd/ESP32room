 ...
 *  Potentio :10 k ohm
 *  Serial read Ver.
 *  SEND commode; potmode; 
 *  SEND usec; PWM_MIN to PWM_MAX integer number
 *  
 */
 //------------------ここから、T-Displayの処理-TTGO-DISPLAY---
#include <TFT_eSPI.h>
#include <SPI.h>
#include <Wire.h>
#include <Button2.h>
...
void setup()
{
  ...
  myservo.setPeriodHertz(100);//  100hz servo
...
void comcontrolemode_disp(){
  String disp_string="";

  if(commode==false){ return;}
  ...
  tft.print(disp_string);
  tft.setCursor(0,40);
  disp_string="COM-Mode";
  tft.print(disp_string);

}
void comcontrolemode(String s_usec){
  ...
  s_len=s_usec.length();
  if(s_len >=3 && s_len<=4){
    pwm_val=s_usec.toInt();
    if(pwm_val>=PWM_MIN && pwm_val<=PWM_MAX){
      Serial.print("commode:");
      Serial.println(pwm_val);
      myservo.writeMicroseconds(pwm_val);
      tft.setCursor(0,80);
      disp_string="PWM :" + String(pwm_val) + "us";
      tft.print(disp_string);  
    }
  }
  
}
String str_command1="commode"; // control from PC
String str_command2="potmode"; // control by Potentio 
...
void loop() {
  ...
  if (Serial.available() > 0){
    str1 = Serial.readStringUntil(';');  // ';' delimiter
    if (str1.equals(str_command1)){
      commode=true;
      comcontrolemode_disp();
    }else if(str1.equals(str_command2)){
      commode=false;
    }else if(str1.length()>=3){
      comcontrolemode(str1);
      Serial.print("Re:");
      Serial.println(str1);
    }
    str1 = "";
